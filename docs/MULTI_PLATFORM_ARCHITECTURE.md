...
The whole strategy hinges on there being one place that decides where data
lives. Originally that decision was scattered: every data operation in
`apps/app/src/lib/*.ts` carried its own `if (isTauriRuntime())` branch with a
Tauri call on one side and hand-written localStorage on the other.

That is now consolidated behind `PupilStorage`:

```ts
// apps/app/src/lib/storage/index.ts — the only runtime check
export function createStorage(): PupilStorage {
  return isTauriRuntime() ? createTauriStorage() : createWebStorage();
}

// every call site
const cards = await getStorage().listCards();
```

| Implementation | Runtime | Persistence | Where |
| --- | --- | --- | --- |
| `createTauriStorage()` | Desktop (`__TAURI_INTERNALS__` present) | SQLite, canonical | `storage/tauri.ts` → `src-tauri/src/*.rs` |
| `createWebStorage()` | Any browser | `localStorage` | `storage/web.ts` |
| *(future)* | Browser PWA / mobile / sync | IndexedDB, native SQLite, remote | new file + one line in `createStorage()` |

Adding a surface means writing one more implementation of this interface. No
call site changes, because no call site knows which one is active.
...
1. **Extract `@pupil/core` + formalize the `Storage` interface.** ✅ *Done —
   see [`packages/core`](../packages/core/README.md).* Domain types, FSRS,
   study-queue rules, shared validation, and day/streak helpers live in a
   platform-free workspace package, and `PupilStorage` defines the persistence
   seam.

   The seam is load-bearing, not just declared. The old per-function
   `if (isTauriRuntime())` branching is gone: `storage/tauri.ts` and
   `storage/web.ts` are two complete implementations, `storage/index.ts` is the
   single place that picks one, and every call site goes through
   `getStorage()`. A third implementation — IndexedDB, native SQLite, or a sync
   adapter — is a new file plus one line in `createStorage()`, with no call-site
   changes. `setStorage()` allows injecting one directly.
